% % lf_CamSim_LightCtrl.m: Post-render light control version
% % 
% % Takes two inputs: imageID (string) and frameId (string), 
% % combines four light groups to create the scene,
% % applies light control to each group per frame, and
% % runs ISETCam simulator to generate captured images.
% % 
% % For flicker simulation, this is part of the "post-render" 
% % light control track, meaning that each light group is 
% % considered one synchronous light source, and
% % the temporal profile is applied as weights per frame per group.
% % 
% % LED parameters are chosen randomly from hyperparameter ranges or
% % fixed values, as defined in lf_RunCam_LightCtrl.m
% % 
% % By changing the weight of the skymap light group, 
% % different ambient lighting conditions can be simulated (day/night).
% % 
% % Input: 
% % - ImageID: '1112160403' etc.
% % - frameId: '01', '02', ..., '10'
% % - wgts: 1x4 vector of weights for the four light groups
% % 
% % Authored by Ayush Jamdar (August, 2025)
% % Based on ISETHDR examples by Zhenyi Liu

% function lf_CameraSim(imageID, frameId, wgts)
