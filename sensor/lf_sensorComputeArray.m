function [sensorCombined,sensors] = lf_sensorComputeArray(sensorArray,oi_spd, ...
    oi_lpd,varargin)
...
p.addRequired('oi_spd',@(x)(isstruct(x(:)) && isequal(x.type,'opticalimage')));
p.addRequired('oi_lpd',@(x)(isstruct(x(:)) && isequal(x.type,'opticalimage')));
...
p.parse(sensorArray,oi_spd, oi_lpd,varargin{:});
...
% LPD LCG
sensors(1) = sensorCompute(sensorArray(1), oi_lpd);

% LPD HCG
sensors(2) = sensorCompute(sensorArray(2), oi_lpd);

% SPD LCG
sensors(3) = sensorCompute(sensorArray(3), oi_spd);
