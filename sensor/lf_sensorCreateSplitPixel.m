function sensorArray = lf_sensorCreateSplitPixel(varargin)
...
for ii=1:2:numel(varargin)
    ...
    if ~isequal(ieParamFormat(str),'arraytype')
        ...
        if strcmp(str, 'exptimespd')
            if ~isempty(SPDLCG)
                SPDLCG.integrationTime = varargin{ii+1};
                SPDLCG.AE = 0;
            end
            if ~isempty(SPDHCG)
                SPDHCG.integrationTime = varargin{ii+1};
                SPDHCG.AE = 0;
            end
        elseif strcmp(str, 'exptimelpd')
            if ~isempty(LPDLCG)
                LPDLCG.integrationTime = varargin{ii+1};
                LPDLCG.AE = 0;
            end
            if ~isempty(LPDHCG)
                LPDHCG.integrationTime = varargin{ii+1};
                LPDHCG.AE = 0;
            end
        else
            if ~isempty(SPDLCG)
