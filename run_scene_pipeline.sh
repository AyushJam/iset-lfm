# Usage: ./run_scene_pipeline.sh <sceneID> <Nframes>
# Steps:
#   A) ./prepare_scene.sh <sceneID>
#   1) MATLAB: lf_DualExposureRecipes(<sceneID>, <Nframes>)
#   2) ./render_splitpixel.sh <sceneID> <Nframes>
#   3) MATLAB: lf_RunCameraLocal(<sceneID>, <Nframes>)
#
	      ...
	      run_and_log() {
			  ...
			  echo
			    echo "---- RUN: $cmd ----" | tee -a "$LOG_FILE"
			      ...
			      # ---- 1) MATLAB: lf_DualExposureRecipes ----
			      run_and_log "matlab -nodisplay -nosplash -batch \"try; \
				        addpath(genpath('~/iset/')); \
					  fprintf('Starting lf_DualExposureRecipes(%s, %d)\\n', '${sceneID}', ${Nframes}); \
					    lf_DualExposureRecipes('${sceneID}', ${Nframes}); \
			      ...
			      # ---- 3) MATLAB: lf_RunCameraLocal ----
			      run_and_log "matlab -nodisplay -nosplash -batch \"try; \
				        addpath(genpath('~/iset/')); \
					  fprintf('Starting lf_RunCameraLocal(%s, %d)\\n', '${sceneID}', ${Nframes}); \
					    lf_RunCameraLocal('${sceneID}', ${Nframes}); \
					    catch e; disp(getReport(e,'extended')); exit(1); end\""
